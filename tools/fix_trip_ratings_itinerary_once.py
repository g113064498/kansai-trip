...
# Restore the itinerary entries that were removed from initialTripData.
old = '''        "2026-11-08": [],
        "2026-11-09": [],
        "2026-11-10": [],
        "2026-11-11": ['''
new = '''        "2026-11-08": [],
        ...
        "2026-11-11": ['''
if old not in s:
    raise SystemExit('itinerary restore marker not found')
s = s.replace(old, new, 1)

# Enrich API-loaded candidate items with verified static rating metadata.
marker = '''        const apiPoolItems = dedupedPoolProducts.map(p => {
            const data = (() => { try { return JSON.parse(p.content || '{}'); } catch { return {}; } })();
            return {
                id: 'api-' + p.id,'''
replacement = '''        const staticRatingAliases = {
        ...
        const apiPoolItems = dedupedPoolProducts.map(p => {
            ...
            return {
                id: 'api-' + p.id,'''
if marker not in s:
    raise SystemExit('apiPoolItems marker not found')
s = s.replace(marker, replacement, 1)

old2 = '''                googleRating: data.googleRating || '',
                tabelogRating: data.tabelogRating || '',
                tabelogUrl: data.tabelogUrl || '',
                ratingChecked: data.ratingChecked || '',
                _productId: p.id'''
new2 = '''                googleRating: data.googleRating || staticMeta.googleRating || '',
                ...
                _productId: p.id'''
if old2 not in s:
    raise SystemExit('rating merge marker not found')
